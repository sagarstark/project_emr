import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

import 'theme.dart';

class Styles {
  const Styles._();

  static TextStyle black16 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.sixteen,
  );

  static TextStyle black14 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.forteen,
  );
  static TextStyle blackBold14 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.forteen,
    fontWeight: FontWeight.bold,
  );
  static TextStyle black12 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.twelve,
  );
  static TextStyle black12w500 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.twelve,
    fontWeight: FontWeight.w500,
  );
  static TextStyle grey10 = GoogleFonts.poppins(
    color: Colors.grey,
    fontSize: Dimens.ten,
  );

  static TextStyle grey12 = GoogleFonts.poppins(
    color: Colors.grey,
    fontSize: Dimens.twelve,
  );
  static TextStyle grey12w500 = GoogleFonts.poppins(
    color: Colors.grey,
    fontSize: Dimens.twelve,
    fontWeight: FontWeight.w500,
  );

  static TextStyle grey14 = GoogleFonts.poppins(
    color: Colors.grey,
    fontSize: Dimens.forteen,
  );

  static TextStyle grey14w500 = GoogleFonts.poppins(
    color: Colors.grey,
    fontSize: Dimens.forteen,
    fontWeight: FontWeight.w500,
  );

  static TextStyle blackBold16 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.sixteen,
    fontWeight: FontWeight.bold,
  );

  static TextStyle black16w500 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.sixteen,
    fontWeight: FontWeight.w500,
  );

  static TextStyle black14w500 = GoogleFonts.poppins(
    color: Colors.black,
    fontSize: Dimens.forteen,
    fontWeight: FontWeight.w500,
  );

  static TextStyle white14w500 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.forteen,
    fontWeight: FontWeight.w600,
  );

  static TextStyle white16 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.sixteen,
  );

  static TextStyle white16w500 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.sixteen,
    fontWeight: FontWeight.w500,
  );

  static TextStyle white14 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.forteen,
  );
  static TextStyle white12 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.twelve,
  );
  static TextStyle white12w500 = GoogleFonts.poppins(
    color: Colors.white,
    fontSize: Dimens.twelve,
    fontWeight: FontWeight.w500,
  );

  static TextStyle whiteBold16 = GoogleFonts.poppins(
      color: Colors.white,
      fontSize: Dimens.sixteen,
      fontWeight: FontWeight.bold);

  static TextStyle whiteBold14 = GoogleFonts.poppins(
      color: Colors.white,
      fontSize: Dimens.forteen,
      fontWeight: FontWeight.bold);
}
